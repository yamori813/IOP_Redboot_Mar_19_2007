...
// Phyical Control Registers
#define I82544_PCTRL    (0<<16)
#define I82544_PSTATUS  (1<<16)
#define I82544_ANA      (4<<16)
#define I82544_LPA      (5<<16)
#define I82544_ANE      (6<<16)
...
#define I82544_PHY_READ 0x08200000
#define I82544_PHY_WRITE 0x04200000
#define POWER_MASK 0xFFFFF7FF
#define I82544_MDIC_READY (1<<28)    
...
static inline void ee_select( int ioaddr, struct i82544 *p_i82544 )
{
    ...
    if (p_i82544->device == 0x1010 ||
        p_i82544->device == 0x1078 ||
        p_i82544->device == 0x1026 ||
        p_i82544->device == 0x105e ||
        p_i82544->device == 0x1079 ||
        p_i82544->device == 0x100f ||
...
get_eeprom_size( struct i82544 *p_i82544 )
{
    ...
    if (p_i82544->device == 0x1078 ||
        p_i82544->device == 0x1026 ||
        p_i82544->device == 0x105e ||
        p_i82544->device == 0x1079 ||
        p_i82544->device == 0x1010 ||
        p_i82544->device == 0x100f ||
        p_i82544->device == 0x100e) {
...
#ifdef CYGNUM_DEVS_ETH_INTEL_I82544_EEPROM_ADDR_DEFAULT_WIDTH
// If the DO pin of the EEPROM is left floating or pulled low
// then we will not have properly determined the width.
// Use the default.
	if (6 != i && 8 != i)
            i = CYGNUM_DEVS_ETH_INTEL_I82544_EEPROM_ADDR_DEFAULT_WIDTH;
#endif
...
i82544_init(struct cyg_netdevtab_entry * ndp)
{
    ...
    if ( p_i82544->mac_addr_ok ) {
        ...
        if (use_bootp == true) {
            os_printf ("Sending BOOTP Requests...can take up to 30secs to timeout...\n");
        }
    } else {
        os_printf ("Invalid MAC Address: %02X:%02X:%02X:%02X:%02X:%02X\n", \
                   p_i82544->mac_address[0], p_i82544->mac_address[1],
                   p_i82544->mac_address[2], p_i82544->mac_address[3],
                   p_i82544->mac_address[4], p_i82544->mac_address[5]);
        os_printf ("You must program MAC via diag menu to enable Ethernet\n");
...
i82544_setup( struct i82544 *p_i82544 )
{
    ...
    ctrl = I82544_PHY_READ | I82544_PCTRL;
    OUTL(ctrl, ioaddr+I82544_MDIC);
    while(!(INL(ioaddr + I82544_MDIC) & I82544_MDIC_READY));
    ctrl = INL(ioaddr + I82544_MDIC) & 0x0000FFFF; //Clear the previous command
    ctrl |= I82544_PHY_WRITE | I82544_PCTRL; // Set the command to write
    ctrl &= POWER_MASK; //Clear the Power Down bit
    OUTL(ctrl, ioaddr+I82544_MDIC);
    while(!(INL(ioaddr + I82544_MDIC) & I82544_MDIC_READY));    
...
//        os_printf("CTRL %08x\n",ctrl);
            ...
            ctrl |= I82544_CTRL_FD;
        {
            ...
            if (phy_pssr & duplex_bit)
                ctrl |= I82544_CTRL_FD;
        }

        // Transfer speed
        if (p_i82544->device == 0x1078) {
            switch((phy_pssr>>14)&3)
            {
            case 1: // 10BaseT
                break;
            case 2: // 100BaseTX
            ...
            unsigned long key)
{
    ...
    for ( last_sg = &sg_list[sg_len]; sg_list < last_sg; sg_list++ )
    {
        ...
        HAL_L2_CACHE_STORE( ((CYG_ADDRESS)from_p) &~(HAL_DCACHE_LINE_SIZE-1),
                          l + (HAL_DCACHE_LINE_SIZE-1) );
#endif
