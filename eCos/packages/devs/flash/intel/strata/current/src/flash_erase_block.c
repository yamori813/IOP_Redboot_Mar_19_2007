...
// Contributors: dmoseley, cebruns, gthomas
// Date:         2001-02-14
// Purpose:
// Description:
...
// FIXME - Handle 'boot' blocks
	...
	// Need to check if we are trying to erase Parameter blocks
	...
	if (strata_query_data_g.num_regions > 1) {
		if (strata_query_data_g.top_parameters == 1) {
			if (eb >= FLASH_P2V(CYGNUM_FLASH_BASE + \
			    ((_si(strata_query_data_g.q_data.region1_blocks)+1) * \
			      strata_query_data_g.large_block))) {
				// We're doing parameter blocks now
				erase_block_size = (strata_query_data_g.small_block);
			}						  
		}
