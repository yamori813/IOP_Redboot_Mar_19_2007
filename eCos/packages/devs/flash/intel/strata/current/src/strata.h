...
// 28FxxxP30 - Advanced StrataFlash    - yes     yes      yes
// 
...
#ifndef CYGHWR_FLASH_WRT_ON
#define CYGHWR_FLASH_WRT_ON()
#endif

#ifndef CYGHWR_FLASH_WRT_RESTORE
...
#define FLASH_BlockLocked   	FLASHWORD( 0x02 )
...
/* J3C devices have unused bits [7:1] set to 1, where J3A have them as 0 - need to mask them off */
#define LOCK_MASK 0x1  
#define INDIVIDUAL_BLOCK_LOCKS (1 << 5)

// Query information
// Modified Nov 17th, 2005 for P30 Flash to support Two Regions with 
// different block sizes.  This will also be applicable to C3 bootblock
// Flash.  cebruns
struct FLASH_query {
    ...
    unsigned char _unused1[2];
	unsigned char ex_query_table[2];
	unsigned char _unused2[16];
    unsigned char device_size;
    unsigned char device_interface[2];
    unsigned char buffer_size[2];
    unsigned char num_regions;   
    unsigned char region1_blocks[2];
    unsigned char region1_size[2];
    unsigned char region2_blocks[2];
    unsigned char region2_size[2];
};

// Extended Query Table information 
// Need to obtain whether or not Individual Block Locks are supported
struct FLASH_ext_query {
    unsigned char id[3];  // P R I
    unsigned char major;  // 1
    unsigned char minor;  // 1 
    unsigned char optional_feature[4]; // Block Locking features within
	unsigned char pad[23];// Pad to 32 byte boundary
};

// Main Structure to keep track of all the query info
struct FLASH_query_data {
	struct FLASH_query q_data;
	struct FLASH_ext_query ext_q_data;
	// Following entries are for internal use 
	unsigned char num_regions;
	unsigned char top_parameters; 
	unsigned char individual_block_locking;
	unsigned char pad[1];
	unsigned int large_block;   
	unsigned int small_block;   
};

extern struct FLASH_query_data strata_query_data_g;
