...
void* volatile __mem_fault_handler CYGBLD_ATTRIB_WEAK = (void *)0;
volatile int __mem_fault CYGBLD_ATTRIB_WEAK;
static volatile unsigned long memCount;

static void
__do_copy_mem8 (cyg_uint8* src, cyg_uint8* dst)
{
...
static void
__do_copy_mem16 (cyg_uint16* src, cyg_uint16* dst)
{
    // Zero memCount is not really an error, but the goto is necessary to
    // keep some compilers from reordering stuff across the 'err' label.
    if (memCount == 0) goto err;
...
static void
__do_copy_mem32 (cyg_uint32* src, cyg_uint32* dst)
{
    // Zero memCount is not really an error, but the goto is necessary to
    // keep some compilers from reordering stuff across the 'err' label.
    if (memCount == 0) goto err;
 ...
 */
int
diag_copy_mem8_safe(cyg_uint8 *dst, cyg_uint8 *src, int count)
{
  memCount = count;
  __do_copy_mem8(src, dst);
 ...
 */
int
diag_copy_mem16_safe (cyg_uint16 *dst, cyg_uint16 *src, int count)
{
  memCount = count;
  __do_copy_mem16(src, dst);
  return count - memCount;      // return number of bytes successfully read
}
 ...
 */
int
diag_copy_mem32_safe (cyg_uint32 *dst, cyg_uint32 *src, int count)
{
  memCount = count;
  __do_copy_mem32(src, dst);
  return count - memCount;      // return number of bytes successfully read
}
                           ...
                           cyg_uint8     *base)
{
    ...
    while ((int)s > 0) {
        ...
        for (i = 0;  i < 16;  i++) {
            if (i < (int)s) {
                //d = p[i];
                if (diag_copy_mem8_safe(&d, &p[i], sizeof(d)) == sizeof(d))
                    (*pf)("%02X ", d & 0xFF);
                else
                    (*pf)(".. ");
            } else {
    ...
    while ((int)s > 0) {
	  ...
	  diag_printf("%08X: ", (CYG_ADDRWORD)p);
        ...
        for (i = 0;  i < 4;  i++) {
            if (i < (int)s/4) {
                //d = p[i];
                if (diag_copy_mem32_safe(&d, &p[i], sizeof(d)) == sizeof(d))
                    diag_printf("%08X ", d);
                else
                    diag_printf("........ ");
            } else {
    ...
    while ((int)s > 0) {
	  ...
	  diag_printf("%08X: ", (CYG_ADDRWORD)p);
        ...
        for (i = 0;  i < 8;  i++) {
            if (i < (int)s/2) {
              //d = p[i];
              if (diag_copy_mem16_safe(&d, &p[i], sizeof(d)) == sizeof(d))
                  diag_printf("%04X ", d);
              else
                  diag_printf(".... ");
	      if (i == 3) diag_printf(" ");
