...
//      ep80219_pci.c
//
//      HAL support code for EP80219 PCI
//
//==========================================================================
...
cyg_hal_plf_pci_init(void)
{
    ...
    hal_pci_inbound_window_mask = (SZ_512M - 1);

#ifdef CYG_HAL_STARTUP_ROM
	{
		cyg_uint32 pci_base = _PCI_MEM_BASE;

		// BAR2 -- the low 512MB of SDRAM
		// 64-bit prefetchable
		pci_base = (pci_base + SZ_512M - 1) & ~(SZ_512M - 1); // Align to 16MB Boundary

		*ATU_IALR2 = ATU_IALR_VAL(SZ_512M);

		*ATU_IATVR2 = CYGARC_PHYSICAL_ADDRESS(SDRAM_BASE);
		*ATU_IABAR2 = pci_base | \
		...
		*ATU_IAUBAR2 = 0;
		...
		// BAR0 -- MMRs size of 16MB
		// 64-bit non-prefetchable
		pci_base = (pci_base + SZ_512M - 1) & ~(SZ_512M - 1); // Align to 16MB Boundary
		*ATU_IALR0 = ATU_IALR_VAL(SZ_512M);
		*ATU_IATVR0 = CYGARC_PHYSICAL_ADDRESS(0xFF000000);
		*ATU_IABAR0 = pci_base | \
					  CYG_PRI_CFG_BAR_MEM_TYPE_64;
		*ATU_IAUBAR0 = 0;
		pci_base += SZ_512M;

		// BAR1 -- used by PCI code to assign space to downstream
		// devices.  Arbitrary size of 128MB.
		pci_base = (pci_base + SZ_128M - 1) & ~(SZ_128M - 1); // Align to 128MB Boundary
		*ATU_IALR1 = ATU_IALR_VAL(SZ_128M);
		*ATU_IABAR1 = pci_base | \
					  CYG_PRI_CFG_BAR_MEM_TYPE_64;
		*ATU_IAUBAR1 = 0;
		pci_base += SZ_128M;
    ...
    *ATU_OIOWTVR = hal_pci_alloc_base_io;
    ...
    hal_pci_physical_memory_base = 0;
    hal_pci_physical_io_base     = _PCI_IO_WINDOW - hal_pci_alloc_base_io;
