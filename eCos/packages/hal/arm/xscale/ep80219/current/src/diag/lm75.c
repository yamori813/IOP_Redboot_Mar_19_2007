...
lm75_read16(cyg_uint8 slave, cyg_uint8 reg, cyg_int16 *val)
{
    ...
    if (!ep80219_diag_i2c_write(slave, &reg, 1, 0))
	return 0;
    
    if (!ep80219_diag_i2c_read(slave, buf, 2, 1))
	return 0;
    *val = (buf[0] << 8) | buf[1];
    return 1;
}
...
lm75_write16(cyg_uint8 slave, cyg_uint8 reg, cyg_int16 val)
{
    ...
    return ep80219_diag_i2c_write_more(buf, 2, 1);
}
...
lm75_read_config(cyg_uint8 slave, cyg_uint8 *val)
{
    ...
    return ep80219_diag_i2c_read(slave, val, 1, 1);
}
...
lm75_write_config(cyg_uint8 slave, cyg_uint8 val)
{
    ...
    return ep80219_diag_i2c_write_more(&val, 1, 1);
}
...
{
    ep80219_diag_i2c_enable();

    diag_printf("\nRegister dump:\n");

    if (!pr_regs("  Sensor 0", 0x48)) {
	ep80219_diag_i2c_disable();
	return;
    }
