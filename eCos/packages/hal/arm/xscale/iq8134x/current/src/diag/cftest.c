...
extern int iq8134x_cf_read_sectors(unsigned long start, unsigned long count, cyg_uint8 *buf);
extern int iq8134x_cf_write_sectors(unsigned long start, unsigned long count, cyg_uint8 *buf);
extern cyg_uint32 iq8134x_cf_blocks(void);
...
cf_test (MENU_ARG arg)
{
    ...
    char *p;
    cyg_uint32 nblocks = iq8134x_cf_blocks();
    cyg_uint32 blk;
    int i;
	...
	if (!iq8134x_cf_read_sectors(blk, 1, save_buf)) {
	    diag_printf("failed (save block %d).\n", blk);
	    return;
	}
	if (!iq8134x_cf_write_sectors(blk, 1, data_buf)) {
	    diag_printf("failed (write data block %d).\n", blk);
	    return;
	}
	memset(temp_buf, 0, 512);
	if (!iq8134x_cf_read_sectors(blk, 1, temp_buf)) {
	...
	for (i = 0; i < 512; i++, p++) {
		...
		iq8134x_cf_write_sectors(blk, 1, save_buf);
		return;
	    }
	}
	if (!iq8134x_cf_write_sectors(blk, 1, save_buf)) {
