...
//
// Author(s):    msalter
// Contributors: msalter, gthomas, dmoseley
// Date:         2000-10-10
...
#define DISPLAY_S       0x92
#define DISPLAY_L       0xc7

/* Backplane Detect Register */
#ifdef __ASSEMBLER__
#define BACKPLANE_DET_REG       0xfe870000
#else
...
#define I2C_BASE0			0x00001680
#define I2C_BASE1			0x000016A0
...
#define IACR_RSV_MASK    0xFFFFC000
...
#define IACR_PATU(x)     ((x) << 0)
#define IACR_SATU(x)     ((x) << 2)
#define IACR_DMA0(x)     ((x) << 4)
#define IACR_DMA1(x)     ((x) << 6)
#define IACR_DMA2(x)     ((x) << 8)
...
#define IACR_REG                REG32(0,IACR_ADDR)
...
/* Provided by IOP_COMMON package */
...
#define SDRAM_BASE   MEMBASE_DRAM
...
//
//  Input: reg0, reg1     -- scratch registers
//         reg2           -- boot time value of config retry flag returned to caller
//         reg_dram_size  -- register holding size of SDRAM
//         vendor_id      -- PCI vendor ID
//         device_id      -- PCI device ID

.macro IOP310_EARLY_PCI_SETUP reg0, reg1, reg2, reg_dram_size, vendor_id, device_id
        ...
        ldr     \reg0, =EBCR_ADDR
        ldr     \reg1, [\reg0]
	and	\reg2, \reg1, #4           // save retry bit for later
        mov     \reg1, #0x0008
        strh    \reg1, [\reg0]
.endm
...
// --------------------------------------------------------------------------
// Software Flags
#define IOP310_CONFIG_RETRY     0x1
#define IOP310_DEV_HIDING       0x2
#define IOP310_PCI_MASTER       0x4
#define IOP310_PCI_PRIVMEM      0x8
