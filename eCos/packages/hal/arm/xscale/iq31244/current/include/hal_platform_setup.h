...
// Date:         2001-12-03
// Purpose:      Intel XScale IQ31244 platform specific support routines
// Description: 
...
#include <cyg/hal/iq31244.h>            // Platform specific hardware definitions
#include <cyg/hal/hal_xscale_iop.h>     // Common IOP definitions
...
// FIXME FIXME FIXME FIXME FIXME FIXME FIXME FIXME FIXME 
	...
	ldr	r2, =PBLR_SZ_4K
	ldr	r1, =IQ31244_UART_ADDR | PBAR_FLASH | PBAR_RCWAIT_20 | PBAR_ADWAIT_20 | PBAR_BUS_8
	str     r1, [r8, #0x10]  // PBIU_PBAR1
	str     r2, [r8, #0x14]  // PBIU_PBLR1

	ldr	r1, =IQ31244_DISPLAY_RIGHT_ADDR | PBAR_FLASH | PBAR_RCWAIT_20 | PBAR_ADWAIT_20 | PBAR_BUS_8
	str     r1, [r8, #0x18]  // PBIU_PBAR2
	str     r2, [r8, #0x1C]  // PBIU_PBLR2

	ldr	r1, =IQ31244_DISPLAY_LEFT_ADDR | PBAR_FLASH | PBAR_RCWAIT_20 | PBAR_ADWAIT_20 | PBAR_BUS_8
	str     r1, [r8, #0x20]  // PBIU_PBAR3
	str     r2, [r8, #0x24]  // PBIU_PBLR3

	ldr	r1, =IQ31244_ROTARY_SWITCH_ADDR | PBAR_FLASH | PBAR_RCWAIT_20 | PBAR_ADWAIT_20 | PBAR_BUS_8
	str     r1, [r8, #0x28]  // PBIU_PBAR4
	str     r2, [r8, #0x2C]  // PBIU_PBLR4

	ldr	r1, =IQ31244_COMPACTFLASH_ADDR | PBAR_FLASH | PBAR_RCWAIT_20 | PBAR_ADWAIT_20 | PBAR_BUS_16
	...
	ldr     r1, =IQ31244_FLASH_ADDR | PBAR_FLASH | PBAR_RCWAIT_20 | PBAR_ADWAIT_20 | PBAR_BUS_16
	...
	ldr	r9, =IQ31244_DISPLAY_RIGHT_ADDR
	ldr	r10,=IQ31244_DISPLAY_LEFT_ADDR
	ldr	r11,=DISPLAY_F
...
icache_boundary:
	...
	str	r1, [r11, #I2C_ISAR]	// Save the value 0x02 (I2C_DEVID) in the register.

	//  Enable I2C Interface Unit - status will be polled
	ldr	r1, =ICR_GENERAL_CALL_DISABLE | ICR_I2C_ENABLE | ICR_SCL_ENABLE
	str	r1, [r11, #I2C_ICR]
	...
	ldr	r0, [r11, #I2C_ISR]	// Load I2C Status Reg into R0
	str	r0, [r11, #I2C_ISR]	// Clear status
	...
	bic	r1, r1, #IDBR_RWMODE	// Clear read bit (bit #0)
	str	r1, [r11, #I2C_IDBR]	// Store to data register

	// Initiate dummy write to set EEPROM pointer to 0
	ldr	r1, [r11, #I2C_ICR]	// read the current Control Register value
	bic	r1, r1,	#ICR_STOP	// No stop bit
	orr	r1, r1,	#ICR_START | ICR_TRANSFER
	str	r1, [r11, #I2C_ICR]	// Store to control register
	...
	ldr	r0, [r11, #I2C_ISR]	// Load I2C Status Reg into R0
	ands	r3, r0, #ISR_TX_EMPTY	// Bit #6 is checked, IDBR Transmit Empty
	beq	0b			// If bit = 0 then branch to 0 and check again
	str	r0, [r11, #I2C_ISR]	// Write back status to clear
...
1:
	...
	orr	r1, r0, #IDBR_RWMODE	// Set read bit (bit #0)
	str	r1, [r11, #I2C_IDBR]	// Store to data register
  ...
  spd_loop:
	...
	beq	i2c_error		// Kick out of SDRAM initialization if timeout occurs
	ldr	r0, [r11, #I2C_ISR]	// Load I2C Status Reg into R0
	ands	r3, r0, #ISR_RX_FULL	// Bit #7 is checked
	beq	0b			// If bit = 0 then branch to 0 and check again
	str	r0, [r11, #I2C_ISR]	// Write back status to clear

	ldr	r1, [r11, #I2C_IDBR]	// Read the byte
  ...
  spd_continue:
	...
	ldr	r1, [r11, #I2C_ICR] 
	bic	r1, r1, #ICR_I2C_ENABLE | ICR_SCL_ENABLE	// Disable I2C unit
	str	r1, [r11, #I2C_ICR]
  ...
  no_ecc2:
	...
	str	r8, [r1]
        ...
        // Save pci_cfg_flags
        //
        // No jumper switches configuring these things.  This board is not
        // an HBA so the relevant settings are:
        // CONFIG_RETRY -- always disabled
        // DEV_HIDING -- always disabled
        // PCI_MASTER -- always enabled
        // PCI_PRIVMEM -- always disabled
        ldr     r0, =IOP321_PCI_MASTER
