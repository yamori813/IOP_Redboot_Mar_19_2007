...
extern int iq31244_cf_read_sectors(cyg_uint32 start, cyg_uint8 count, cyg_uint8 *buf);
extern int iq31244_cf_write_sectors(cyg_uint32 start, cyg_uint8 count, cyg_uint8 *buf);
extern cyg_uint32 iq31244_cf_blocks(void);
...
cf_test (MENU_ARG arg)
{
    ...
    char *p;
    cyg_uint32 nblocks = iq31244_cf_blocks();
    cyg_uint32 blk;
    int i;
	...
	if (!iq31244_cf_read_sectors(blk, 1, save_buf)) {
	    diag_printf("failed (save block %d).\n", blk);
	    return;
	}
	if (!iq31244_cf_write_sectors(blk, 1, data_buf)) {
	    diag_printf("failed (write data block %d).\n", blk);
	    return;
	}
	memset(temp_buf, 0, 512);
	if (!iq31244_cf_read_sectors(blk, 1, temp_buf)) {
	...
	for (i = 0; i < 512; i++, p++) {
		...
		iq31244_cf_write_sectors(blk, 1, save_buf);
		return;
	    }
	}
	if (!iq31244_cf_write_sectors(blk, 1, save_buf)) {
