...
#if 1
// I2C 0
#define _ICR   I2C_ICR0
#define _ISR   I2C_ISR0
#define _ISAR  I2C_ISAR0
#define _IDBR  I2C_IDBR0
#else
// I2C 1
...
int
iq31244_diag_i2c_read(cyg_uint8 slave, cyg_uint8 *buf, int nbytes, int eot)
{
...
int
iq31244_diag_i2c_read_more(cyg_uint8 *buf, int nbytes, int eot)
{
    while (--nbytes >= 0) {
	if (nbytes == 0 && eot)
	    *_ICR = ICR_STOP_TRANSFER;
...
int
iq31244_diag_i2c_write(cyg_uint8 slave, cyg_uint8 *buf, int nbytes, int eot)
{
    *_IDBR = slave << 1;
    *_ICR = ICR_START_TRANSFER;
    if (!wait_for_empty())
...
int
iq31244_diag_i2c_write_more(cyg_uint8 *buf, int nbytes, int eot)
{
    while (--nbytes >= 0) {
	*_IDBR = *buf++;
	if (nbytes == 0 && eot)
