...
//      iq31244_cf.c
//
//      HAL compact flash support code for Intel XScale IQ31244
//
//==========================================================================
...
#define HAL_IDE_READ_UINT8( __regno, __val )  \
    (__val) = *(volatile cyg_uint8 *)(IQ31244_IDE_BASE + (__regno))

#define HAL_IDE_WRITE_UINT8( __regno, __val )  \
    *(volatile cyg_uint8 *)(IQ31244_IDE_BASE + (__regno)) = (__val);
...
int
iq31244_cf_read_sectors(cyg_uint32 start, cyg_uint8 count, cyg_uint8 *buf)
{
    int  i, j;
    cyg_uint8 *p;
...
int
iq31244_cf_write_sectors(cyg_uint32 start, cyg_uint8 count, cyg_uint8 *buf)
{
    int  i, j;
    cyg_uint8 *p;
...
cf_write(int argc, char *argv[])
{
    ...
    if (!iq31244_cf_write_sectors(sector_addr, nsects, (cyg_uint8 *)mem_addr)) {
...
cf_read(int argc, char *argv[])
{
    ...
    if (!iq31244_cf_read_sectors(sector_addr, nsects, (cyg_uint8 *)mem_addr)) {
...
{
    if (!iq31244_cf_read_sectors(start_sector, nsects, p))
	return 0;
    return nsects;
}
