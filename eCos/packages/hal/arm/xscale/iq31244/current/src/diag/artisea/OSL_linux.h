...
#include <linux/spinlock.h>  /* spin_lock support */
#include <asm/system.h>      /* spin_lock support */
...
#define OsSpinLock(lock, flags) spin_lock_irqsave(lock, flags)
#define OsSpinUnlock(lock, flags) spin_unlock_irqrestore(lock, flags)
#define OsSpinlockInit(lock) spin_lock_init(lock)
