...
* --------------------
* 08/30/03  ejb  Created for the IQ31244 Intel Artisea Evaluation Board
* 
*/
...
void artisea_test()
{
	...
	diag_printf ("    *************************************************\n");
        ...
        // Arbitrarily place dmaDescTable at address 8MB
        dmaDescTable = (DDT_ENTRY_T *)(0x800000);
        memset(dmaDescTable, 0, (sizeof(DDT_ENTRY_T) * DDT_MAX_ENTRIES));
...
static int do_read_write(int adapterTag_i, int devTag_i, int sector)
{
	...
	dmaTableXlate = (UINT64)((UINT32)&dmaDescTable[0]);
...
/* debug print */
static void print_local_data_struct()
{
int ctr, i;
