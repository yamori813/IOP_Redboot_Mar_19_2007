...
_real_platform_setup:
	...
	ldr	r11, =I2C_BASE0		// Load the I2C Base Address in r11.
	...
	str	r3, [r11, #I2C_ISAR]	// Save the value 0x02 (I2C_DEVID) in the I2C Slave Address Register.

	//  Setup I2C Clock Count Register
	ldr     r3, =0x0000014d		// Set for 5.05 us transition time at 66MHz (0x14D = 333).
	str	r3, [r11,#I2C_ICCR]	// Save the value in the register.

	//  Enable I2C Interface Unit - status will be polled
	ldr	r1, =ICR_GENERAL_CALL_DISABLE	// Disable General Call (will be master)
	ldr	r3, =ICR_I2C_ENABLE	// Enable I2C unit ).
	orr	r1, r3, r1		// OR the two and store in R1
	ldr	r3, =ICR_SCL_ENABLE	// Enable I2C Clock Generator disabled
	orr	r1, r3, r1		// OR the two and store in R1
	str	r1, [r11, #I2C_ICR]	// Save the value to the Control Register.
	...
	beq	i2c_error		// Kick out of SDRAM initialization if timeout occurs
	ldr	r0, [r11, #I2C_ISR]	// Load I2C Status Reg into R0
	ands	r3, r0, #ISR_RX_FULL	// Bit #6 is checked: IDBR Transmit Empty
	beq	0b		                    
	str	r0, [r11, #I2C_ISR]	// Write back status to clear
...
SDRAM_DRIVE_2_BANK_X8:
	...
	ldr	r1, [r11, #I2C_ICR] 
	bic	r1, r1, #ICR_I2C_ENABLE	// Disable I2C unit
	bic	r1, r1, #ICR_SCL_ENABLE	// Disable I2C clock generator
	str	r1, [r11, #I2C_ICR]	// Store to control register
   ...
   init_dram:
	...
	IOP310_EARLY_PCI_SETUP  r0, r1, r13, r4, 0x113C, 0x0700

	// Determine boot time config flags.
        ldr     r0, =0

        // Config Retry
        // -- Set based on EBCR
        cmp     r13, #0
	ldrne	r1, =IOP310_CONFIG_RETRY
        orrne   r0, r0, r1

        // Device Hiding
        // -- All devices are made private in IOP310_EARLY_PCI_SETUP
	ldr	r1, =IOP310_DEV_HIDING
        orr     r0, r0, r1

        // Private Memory
        // -- Unimplemented on the IQ80310

        // PCI Master Mode
        // -- Set based on BP_HOST_BIT in the BACKPLANE_DET_REG
        ldr     r1, =BACKPLANE_DET_REG
        ldr     r2, [r1]
        ldr     r1, =BP_HOST_BIT
        ands    r2, r2, r1
	ldrne	r1, =IOP310_PCI_MASTER
        orrne   r0, r0, r1

        // Save the flags in R13 so we can store it to RAM once initialized
        mov     r13, r0
  ...
  667:
	...
	HEX_DISPLAY r0, r1, DISPLAY_A, DISPLAY_0
