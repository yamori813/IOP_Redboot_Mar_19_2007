...
cyg_hal_plf_pci_init(void)
{
...
#ifdef CYG_HAL_STARTUP_ROM
    if (((hal_pci_cfg_flags & IOP321_CONFIG_RETRY) == 0) || !cyg_hal_plf_wait_for_bios())
    {
        // No configuration by a Host BIOS, take over Master role
        hal_pci_cfg_flags |= IOP321_PCI_MASTER;

	*ATU_IABAR2 = SDRAM_PHYS_BASE | (*ATU_IABAR2 &~ CYG_PRI_CFG_BAR_MEM_MASK);
	*ATU_IAUBAR2 = 0;

	// Outbound window will be set based on the memory reserved
        // by inbound window 1
	*ATU_IABAR1 = _PCI_MEM_BASE | (*ATU_IABAR1 &~ CYG_PRI_CFG_BAR_MEM_MASK);
	*ATU_IAUBAR1 = 0;
    }
#endif  // CYG_HAL_STARTUP_ROM
    ...
    *ATU_OIOWTVR = hal_pci_alloc_base_io;
    ...
    hal_pci_physical_memory_base = _PCI_MEM_BASE - *ATU_OMWTVR0;
    hal_pci_physical_io_base     = _PCI_IO_BASE - *ATU_OIOWTVR;
