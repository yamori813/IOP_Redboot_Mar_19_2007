...
1:
  ...
  no_ecc1:
	...
	str     r0, [r1]
	mov	r0, #0x0
	ldr     r1, =ATU_IABAR0   // Non-prefetchable - 32-bit space - required for MU usage
	str     r0, [r1]
	ldr     r1, =ATU_IABAR1   // Non-prefetchable - 32-bit space
	str     r0, [r1]
	mov	r0, #0xc
	ldr     r1, =ATU_IABAR2   // Prefetchable - 64-bit space
        str     r0, [r1]
	mov	r0, #0
        ldr     r1, =ATU_IAUBAR0
	str     r0, [r1]
  ...
  no_ecc2:
	...
	// Determine boot time config flags.
        ...
        // -- Set based on PCSR
        cmp     r13, #0
	ldrne	r1, =IOP321_CONFIG_RETRY
        orrne   r0, r0, r1

        // Device Hiding
        // -- Unfortunately we cannot know the status of device hiding as
        //    implemented on the IQ80321.  Assume it is enabled.
	ldr	r1, =IOP321_DEV_HIDING
        orr     r0, r0, r1

        // Private Memory
        // -- Unimplemented on the IQ80321

        // PCI Master Mode
        // -- Set based on PCSR Config retry.
        //    If retry is enabled, assume not master
        cmp     r13, #0
	ldreq	r1, =IOP321_PCI_MASTER
        orreq   r0, r0, r1
...
hal_pci_cfg_flags:   .long   0  // Boot-time value of PCI configration flags
        .endm                                        
