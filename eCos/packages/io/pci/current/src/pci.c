...
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("DEVICE ACTIVE: MEM bar, BAR address is: 0x%08x\n", tmp_addr);
#endif
...
#ifdef CYGPKG_IO_PCI_DEBUG
					diag_printf("Update memory base to 0x%08x%08x ", \
					(cyg_pci_memory_base >> 32) & 0xFFFFFFFF, (cyg_pci_memory_base & 0xFFFFFFFF));
					diag_printf ("from BAR: %d\n", i);
#endif
                }
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("(((Adding 0x%08x%08x to tmp: ", \
				(HAL_PCI_PHYSICAL_MEMORY_BASE >> 32) & 0xFFFFFFFF, (HAL_PCI_PHYSICAL_MEMORY_BASE & 0xFFFFFFFF));
				diag_printf ("0x%08x%08x\n", (tmp_addr >> 32) & 0xFFFFFFFF, (tmp_addr & 0xFFFFFFFF));
#endif
...
{
    cyg_uint16 bus = CYG_PCI_DEV_GET_BUS(cur_devid);
...
{
#ifdef CYGPKG_IO_PCI_DEBUG	
		diag_printf ("****Setting PCI_Mem Base to 0x%08x\n", base);
#endif
...
{
#ifdef CYGPKG_IO_PCI_DEBUG	
		diag_printf ("****Setting PCI_IO Base to 0x%08x\n", base);
#endif
...
#ifdef CYGPKG_IO_PCI_DEBUG
	diag_printf ("\nInside ALLOCATE_MEM_PRIV\n");
	diag_printf ("Base_Map[%d]: 0x%08x and HAL_PCI_PHYSICAL: 0x%08x\n", \
		bar, dev_info->base_map[bar], HAL_PCI_PHYSICAL_MEMORY_BASE);

	diag_printf ("Assigning Aligned_Addr of: 0x%08x\n", aligned_addr);
	diag_printf ("Updating Base Resource to: 0x%08x\n", *base);
#endif
		       ...
		       cyg_uint8 *next_bus )
{
	...
	cyg_pcihw_write_config_uint16(bus, devfn, CYG_PCI_CFG_COMMAND, 0);	
...
#ifdef CYGPKG_IO_PCI_DEBUG
		diag_printf (">>>>BaseRegisters for Device<<<<\n");
		diag_printf ("BAR[0]: 0x%08x\n", dev_info.base_address[0]);
		diag_printf ("BAR[1]: 0x%08x\n", dev_info.base_address[1]);
		diag_printf ("BAR[2]: 0x%08x\n", dev_info.base_address[2]);
		diag_printf ("BAR[3]: 0x%08x\n", dev_info.base_address[3]);
		diag_printf ("BAR[4]: 0x%08x\n", dev_info.base_address[4]);
		diag_printf ("BAR[5]: 0x%08x\n", dev_info.base_address[5]);

		diag_printf ("CPU BAR[0]: 0x%08x\n", dev_info.base_map[0]);
		diag_printf ("CPU BAR[1]: 0x%08x\n", dev_info.base_map[1]);
		diag_printf ("CPU BAR[2]: 0x%08x\n", dev_info.base_map[2]);
		diag_printf ("CPU BAR[3]: 0x%08x\n", dev_info.base_map[3]);
		diag_printf ("CPU BAR[4]: 0x%08x\n", dev_info.base_map[4]);
		diag_printf ("CPU BAR[5]: 0x%08x\n", dev_info.base_map[5]);
		diag_printf ("---------------------------------\n");
#endif
	...
	if (header_type != CYG_PCI_HEADER_BRIDGE) {
	    ...
	    *next_bus += 1;
	    ...
	    cyg_pcihw_write_config_uint8(bus, devfn, CYG_PCI_CFG_SUB_BUS,
					 dev_info.header.bridge.sub_bus);
#ifdef CYGPKG_IO_PCI_DEBUG
			diag_printf ("***In Bridge Configuration Routine\n");
			diag_printf ("***  Primary Bus: %d\n", bus);
			diag_printf ("***  Secondary Bus: %d\n", dev_info.header.bridge.sec_bus);
			diag_printf ("*** Configured Subordinate Bus: %d\n", dev_info.header.bridge.sub_bus );
			diag_printf ("*** IO_Base Before : 0x%08x\n", io_start);
			diag_printf ("*** Mem_Base Before: 0x%08x\n", mem_start);
#endif
	    // Did sub bus configuration use any I/O?
	    if (cyg_pci_io_base > io_start) {
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("*** IO_Base After: 0x%08x\n", cyg_pci_io_base);
#endif
		...
		if ((dev_info.header.bridge.io_base & 0x0f) == 0x01) {
...
#ifdef CYGPKG_IO_PCI_DEBUG
					diag_printf ("Writing: 0x%08x to 0x%08x for IO Space Upper 16 bits\n", \
						dev_info.header.bridge.io_base_upper16, CYG_PCI_CFG_IO_BASE_UPPER16);
#endif
		    cyg_pcihw_write_config_uint16(bus, devfn, CYG_PCI_CFG_IO_BASE_UPPER16,
						 dev_info.header.bridge.io_base_upper16);
		    ...
		    cyg_pcihw_write_config_uint16(bus, devfn, CYG_PCI_CFG_IO_LIMIT_UPPER16,
						 dev_info.header.bridge.io_limit_upper16);
#ifdef CYGPKG_IO_PCI_DEBUG
					diag_printf ("Writing: 0x%08x to 0x%08x for IO Space Upper 16 Limit\n", \
						dev_info.header.bridge.io_limit_upper16, CYG_PCI_CFG_IO_LIMIT_UPPER16);
#endif
		}
...
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("Writing: 0x%08x to 0x%08x for IO Base\n", \
						dev_info.header.bridge.io_base, CYG_PCI_CFG_IO_BASE);
#endif
...
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("Writing: 0x%08x to 0x%08x for IO Limit\n", \
						dev_info.header.bridge.io_limit, CYG_PCI_CFG_IO_LIMIT);
#endif
		cyg_pcihw_write_config_uint8(bus, devfn, CYG_PCI_CFG_IO_LIMIT,
					     dev_info.header.bridge.io_limit);
...
#ifdef CYGPKG_IO_PCI_DEBUG
		diag_printf ("No IO Space used behind bridge\n");
#endif
...
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("Writing: 0x%08x to 0x%08x for Mem Base\n", \
						dev_info.header.bridge.mem_base, CYG_PCI_CFG_MEM_BASE);
#endif
...
#ifdef CYGPKG_IO_PCI_DEBUG
				diag_printf ("Writing: 0x%08x to 0x%08x for Mem Limit\n", \
						dev_info.header.bridge.mem_limit, CYG_PCI_CFG_MEM_LIMIT);
#endif
		...
		// Disable by making limit < base
		{
		    cyg_uint16 tmp_word;
#ifdef CYGPKG_IO_PCI_DEBUG
					diag_printf ("Disabling Prefetchable memory across the bridge\n");
#endif
		    ...
		    cyg_pcihw_write_config_uint16(bus, devfn, CYG_PCI_CFG_PREFETCH_LIMIT,
						 tmp_word);
#ifdef CYGPKG_IO_PCI_DEBUG
					diag_printf ("Writing: 0x%08x to 0x%08x for Prefetch Limit\n", \
						tmp_word, CYG_PCI_CFG_PREFETCH_LIMIT);
#endif
		    ...
		    cyg_pcihw_write_config_uint16(bus, devfn, CYG_PCI_CFG_PREFETCH_BASE,
						 tmp_word);
#ifdef CYGPKG_IO_PCI_DEBUG
					diag_printf ("Writing: 0x%08x to 0x%08x for Prefetch Base\n", \
						tmp_word, CYG_PCI_CFG_PREFETCH_BASE);
#endif
				...
				diag_printf ("No Memory used behind bridge\n");
#endif
